...
import { Currency, CurrencyAmount, TokenAmount, Trade, Token, Price, ETHER } from '@quantumdex/sdk'
  ...
  const onCurrencySelection = useCallback(
    (field: Field, currency: Currency) => {
      dispatch(
        selectCurrency({
          field,
          currencyId: currency instanceof Token ? currency.address : currency === ETHER ? 'AME' : '',
        }),
      )
    },
    [dispatch],
...
function parseCurrencyFromURLParameter(urlParam: any): string {
  if (typeof urlParam === 'string') {
    ...
    if (urlParam.toUpperCase() === 'AME') return 'AME'
    if (valid === false) return 'AME'
  }
  return ''
}
